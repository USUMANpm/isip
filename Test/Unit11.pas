...
begin
  ...
  B := FAnswerCount;
  Ini.WriteInteger('Тест№3', 'Результаты тестирования', B);
...
begin
  if FTestOK then Form6.Show  // отбражаем следующую лекцию
             else Form5.Show; // возвращаемся к первой
...
begin
  Ini := TIniFile.Create(ExtractFilePath(ParamStr(0))+'lala.ini');
  Edit1.Text := IntToStr(Ini.ReadInteger('Тест№3', 'Rez', 0));
end;

procedure TForm11.FormDestroy(Sender: TObject);
begin
  Ini := TIniFile.Create(ExtractFilePath(ParamStr(0))+'lala.ini');
  Ini.WriteInteger('Тест№3', 'Rez', B);
end;
