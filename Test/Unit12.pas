...
begin
  ...
  B := FAnswerCount;
  Ini.WriteInteger('Тест№4', 'Результаты тестирования', B);
begin
...
begin
  if FTestOK then Form7.Show  // отбражаем следующую лекцию
             else Form6.Show; // возвращаемся к первой
...
begin
  Ini := TIniFile.Create(ExtractFilePath(ParamStr(0))+'lala.ini');
  Edit1.Text := IntToStr(Ini.ReadInteger('Тест№4', 'Rez', 0));
end;

procedure TForm12.FormDestroy(Sender: TObject);
begin
  Ini := TIniFile.Create(ExtractFilePath(ParamStr(0))+'lala.ini');
  Ini.WriteInteger('Тест№4', 'Rez', B);
end;
end.
